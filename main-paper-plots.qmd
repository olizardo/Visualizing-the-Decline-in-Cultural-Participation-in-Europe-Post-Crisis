...
```{r Generating data for plotting}
     ...
     likert.all <- likert(eurobar.bin[3], grouping = eurobar.bin$country_year)
     likert.lhs <- likert(dat.lhs[3], grouping = dat.lhs$country_year)
     likert.hs <- likert(dat.hs[3], grouping = dat.hs$country_year)
     likert.uni <- likert(dat.uni[3], grouping = dat.uni$country_year)
```

```{r Generating plots}
     plot.dat <- function(x, t = "") {
       p <- plot(x,
            colors = wes_palette("Royal1"), center = 1.5, text.size = 3,
            group.order = c("Portugal_2007","Portugal_2013", 
                            "Greece_2007", "Greece_2013", 
                            "Spain_2007", "Spain_2013",
                            "Italy_2007", "Italy_2013")
       )
       p <- p + theme_minimal()
    ...
    p1 <- plot.dat(likert.all, t = "All Respondents")
    p2 <- plot.dat(likert.lhs, t = "Finished Education at Fifteen or Younger (> High School)")
    p3 <- plot.dat(likert.hs, t = "Finished Education Between Sixteen and Nineteen (High School/Vocational)")
    p4 <- plot.dat(likert.uni, t = "Completed Education at Twenty or Older (University)")
```

```{r Combining plots and saving composite plot}
    p.list <- list(p1, p2, p3, p4)
    p <- wrap_plots(p.list, ncol = 1) + plot_layout(guides = "collect") &
